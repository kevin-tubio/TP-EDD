...
class Indexador():
    ...
    def armar_indices(self):
        ...
        with open(nombre_archivo, "r", encoding="utf-8") as lector_csv:
            ...
            self.__indice_frase_id = self.__ordenar_claves(self.__generar(pares_frase_id, indice_frase_id))
            self.__indice_palabra_id = self.__ordenar_claves(self.__generar(pares_palabra_id, indice_palabra_id))
            self.__indice_id_usuario = self.__ordenar_claves(self.__generar(pares_id_usuario, indice_id_usuario))
            self.__indice_id_tweet = self.__ordenar_claves(self.__generar(pares_id_tweet, indice_id_tweet))
            self.__indice_fecha_id = self.__ordenar_claves(self.__generar(pares_fecha_id, indice_fecha_id))
            #ordenar valores
    ...
    #         otro_diccionario.setdefault(clave)
    #         ordenado = tuple(sorted(list(valor)))
    #         otro_diccionario[clave] = ordenado
